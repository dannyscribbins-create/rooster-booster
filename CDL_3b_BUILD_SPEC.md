...
| **(e)** | **DERIVABLE from stored data, but never actually derived** | Needs a **derivation**, in one place, drift-guarded. |

> **⚠ (e) WAS ADDED AFTER PHASE 6 SHIPPED, and it is the most invisible of the five.** `contractor_about.google_place_id` was **populated** in production and already driving the About modal's Google rating; `contractor_settings.review_url` was **empty**; and the review card needed a URL **constructed from the former**. Schema complete, admin panel complete, both fields editable — and the surface still had nothing to show, because **the value it needed did not exist as a stored field at all.**
>
> **A classification built from the schema and the admin panel cannot see this**, because both look finished. The question that exposes it is: *"can this value be CONSTRUCTED from something already stored?"* — and it must be asked about the fields that look empty, not the ones that look complete.
...
- **🔴 PRE-LAUNCH — THE ADMIN PANEL HAS NO SIGN-OUT CONTROL, AND ITS SESSIONS ARE NOW 30 DAYS.** *(Found in Phase 4. Danny-confirmed: the D6 text was wrong about these sites.)* D6 described logout as replacing "~10 separate inline implementations scattered through the admin panel". They are **401 expiry handlers** — `on401 = () => { clearAdminToken(); setLoggedIn(false); }` — not logout controls. ⚠ **AND THERE ARE NOT TEN. Re-counted 2026-08-27: 21 `status === 401` guard sites across 6 files** — `AdminReferrers` 8, `AdminEngagement` 5, `AdminCashOuts` 4, `AdminCampaigns` 2, `AdminDashboard` 1, `AdminActivityLog` 1 — driven through 4 `on401` helpers plus 3 inline copies. (22 by *comparison* count; `AdminEngagement.jsx:338` tests two responses on one line. 21 is the number of distinct handling sites, and the one to size against.) `AdminApp.jsx:224` still says "roughly ten sites"; that comment inherits this error and is corrected with the D6 record here rather than in six component files. **The correction that mattered was the NATURE of these sites, and it was made — but the COUNT rode along uncorrected and is off by more than 2×, which is the figure any step-up or shared-fetch-wrapper session will size against.** Grepping `admin/` for a sign-out affordance returns **nothing**: only the referrer app (the Contact Support + Sign Out block in `ProfileTab.jsx`) and `SuperAdminShell.jsx:70` have one. Phase 4 shipped `POST /api/logout` and wired both of those, and left `logoutAdmin()` in `src/utils/authStorage.js` **exported, tested, and deliberately without a caller**.
...
- **PRE-LAUNCH — `err.message` leaked in ~40 500-responses.** Concentrated in `server/routes/account.js` (15 sites) plus `referrer.js:1158`, `admin/cashouts.js:37,156`, `admin/referrers.js:68,114,127`. A Security Standards violation; `referrer.js:1127-1137` documents this exact class being swept twice before and missing a third door. Systematic — its own item, not folded into a feature session.
...
- ~~**3c — `useAdminPermissions.js` still drops `is_attributable` and `rep_revenue_visibility`.** Phase 5 surfaced **`is_field_rep` only**, deliberately scoped: the other two belong to 3c's CD-7 revenue gate, and a payload nothing reads is API surface acquired by accident. `server/test/repRouting.test.js` asserts both are **absent** from the two auth payloads, so widening them is a deliberate act rather than a one-word edit. CDL_3a §8's note about the context dropping all three **stays open**.~~ ✅ **DONE in C/DL-3c Phase 2a — and the deliberate act it asked for was to LEAVE THE SCOPE PIN ALONE.** The hook now carries all four dropped fields, with **zero server change**: `GET /api/admin/me` has selected and returned them since C/DL-3a, and that is the endpoint the hook reads. **The two AUTH payloads were not widened and must not be** — `repRouting.test.js`'s scope-pin case is still green and still a working fence, and the reason is recorded at its own site. ⚠ **Do not read "3c needed the flags" as "3c should have widened this."** It needed them somewhere else, and they were already there. The rep tree reads them through `RepCapabilitiesContext` (`src/hooks/useAdminPermissions.js`), a second context with **no default value**, mounted on `src/App.jsx`'s rep branch — because `AdminPermissionsContext`'s default is what made a flag-off gate test pass against completely unwired code (vacuity shape #10, guard-proofed in `src/components/rep/repCapabilitiesSeam.test.jsx`).
...
- **🔴 FIFTH VACUITY INSTANCE — AND THE FIRST TO REACH PRODUCTION.** *(Found 2026-08-14, after Phase 6 deployed.)* The 6C `DashboardTab` test asserted `reviewMessage` and `reviewButtonText`. **Both are DEFAULTED**, so both render whatever happens. **Nothing exercised `reviewUrl`, which is NOT defaulted** — so nothing saw that the review card shipped rendering a button whose `window.open(null)` resolved to **`app.roofmiles.com/null`** (`null` stringifies to `"null"` per USVString conversion and resolves against the document).
  - **THE RULE: when a value is deliberately allowed to be absent, THE ABSENT CASE IS THE PRIMARY TEST.** A test that asserts a component's **defaulted** values cannot see a bug in its **non-defaulted** ones — and the defaulted ones are the easy, obvious things to assert, which is why this is the shape it keeps taking.
  - **Two more of the same class were introduced in 6C and found by the same pass:** `AnnouncementPopup` and `CashOutTab` rendered `<img src={branding.logoUrl}>` unguarded, because the hardcoded logo they replaced could never be null. Every other `logoUrl` consumer guards.
- **🔴 A SWEEP PROVES A STRING IS ABSENT. IT PROVES NOTHING ABOUT WHETHER THE CODE STILL RUNS.** *(From 6C's `[Company]` substitution; found 2026-08-14, after it had shipped.)* **This is a DIFFERENT SHAPE from the vacuity findings above, and it is recorded separately for that reason.**
  - The substitution referenced `branding` inside `resolveMessage` — a **module-level** function, where that name is not in scope. `AnnouncementPopup` threw a `ReferenceError` on **every render** with that template.
  - **THE SWEEP WAS CORRECT.** It proved the Accent literal was gone, and it was. The component simply did not run any more. **No sweep, no grep, and no static check could have caught this** — the file passed lint, passed the literal sweep, and parsed fine. **Only rendering it could.**
  - **THE RULE: any file a sweep touches needs at least one render test, however trivial.** A sweep is a statement about text; a render is the only statement about behaviour. Phase 6 swept nine files and rendered six of them, and the crash landed in one of the three.
- **🔴 THE FILES-LIST LIMIT — THE BIGGER OF THE TWO SWEEP GAPS.** *(Recorded 2026-08-14 alongside the formatting gap, and deliberately NOT as a footnote to it.)* Every sweep in this repo iterates a **hand-maintained list of file paths**. Normalisation fixed **how values are compared**; **nothing addresses WHICH FILES ARE LOOKED AT.**
  - **A hand-maintained list is precisely the thing that goes stale silently.** Every new file is invisible to the sweep until someone remembers to add it, and **nothing announces the omission** — the sweep reports green over a shrinking fraction of the codebase.
  - **⚠ THIS IS WHY THE POST-FIX RE-SWEEP FOUND NOTHING NEW, AND THAT RESULT MUST NOT BE READ AS "THE REPO IS CLEAN."** It is evidence only that **the listed files** are clean. The two known remaining hits (`CompanyDetailsSettings`'s admin placeholder, `PrivacyPolicy`'s legal text) sit outside every list and are invisible to the sweeps regardless of normalisation.
  - **Options for the fix:** walk a directory tree rather than a list (as 6D's `CONTRACTOR_CONFIG` importer check already does), or assert the list's completeness against a glob so a new file fails until it is classified.
- **🔴 THE FORMATTING-NOT-VALUES SWEEP GAP — IT UNDERMINES EVERY SWEEP WRITTEN THIS SESSION, INCLUDING THE ONES THAT REPORTED CLEAN.** *(Same date.)* `ContactModal` displayed the right contractor's phone number and **dialled the wrong one**: the visible text was rewired in 6C, the `tel:` href was not, and **the sweep missed it because its needle was the DASHED rendering while a `tel:` URI carries bare digits.** Live in production, on the tap target a homeowner on a phone actually presses — and *worse than the original bug*, because it looked correct to anyone checking visually.
  - **A LITERAL SWEEP MATCHES FORMATTING, NOT VALUES.** Both the client and server sweeps had the hole. Both now **normalise before comparing**: phone needles compare digit-runs (dashed, bare, dotted, parenthesised, country-prefixed all match), URL needles strip scheme / `www.` / trailing slash, and names and emails compare as-is — email being the one single-rendering value here.
  - **Verified by injection:** all four `tel:` renderings are caught by the normalised sweep; the old one caught only the dashed form.
  - **A repo-wide normalised re-sweep found no other production site.** Reported as-is rather than padded: the only hits are the admin placeholder in `CompanyDetailsSettings` and the legal page in `PrivacyPolicy`, both dashed and both already out of scope by ruling. **See the files-list entry above for why that result is weaker evidence than it looks.**
...
### Recorded from the review-URL correction

- **📎 `review_url` NOW DERIVES FROM `google_place_id`, WITH THE OVERRIDE PRESERVED.** Precedence: **`review_url` if set → else derive from `google_place_id` → else null.** Canonical derived form `https://search.google.com/local/writereview?placeid=<ID>`, URL-encoded, in `resolveBrandingTheme` (both copies, drift-guarded).
  - **The Place ID is canonical because it is a stable identifier entered once**, while a write-review URL is a formatted string whose shape Google controls. One input, one field to maintain, no way for the two to disagree.
  - **⚠ THE OVERRIDE IS LOAD-BEARING, NOT A COURTESY.** A `g.page/r/…` short link is **CID-derived, NOT a Place ID**: it cannot be regenerated from a Place ID and a Place ID cannot be recovered from it. A contractor holding one — or pointing at a non-Google destination — must keep it.
  - `loadContractorBranding` now LEFT JOINs `contractor_about`, because the two fields live in different tables (`google_place_id` on `contractor_about` via Company Details → About Us; `review_url` on `contractor_settings` via Branding).
- **🚫 `social_google` IS AN EXPLICIT NON-GOAL.** It is a Google Business **profile** link used in campaign email footers. A profile link and a write-review link are **different destinations**; conflating them would send campaign readers to a review form. Recorded so nobody folds it in as "the other Google field".
- **🟡 TWO EDITORS WRITE `google_place_id` — RECORDED, NOT FIXED.** `AdminAboutUs.jsx:98` and `CompanyDetailsSettings.jsx:280`, both POSTing `/api/admin/about`. A live divergence: two screens can disagree about one field. **Admin-panel scope — belongs with the Admin Panel Brand Retirement build.**
- **✅ DEAD `google_place_id` REMOVED from the experience-prompt payload.** `GET /api/referrer/experience-prompt` selected it (with a join) and returned it; `ExperiencePopup` never read it, taking its review link from the branding context instead. Unread payload is API surface acquired by accident — same reasoning that scoped Phase 5's auth change to `is_field_rep` alone.
...
- **🔴 DECISION E INPUT — there is NO reactivation path. Deactivation is a one-way door.** *(Found during Phase 3 production verification.)* `AdminTeamSettings.jsx:1833` gates the deactivate control on `m.active`, so an inactive row renders the edit pencil alone — but the gap is **not** cosmetic, and adding a button would not close it. `team.js:555`'s `UPDATE team_members SET active = false` is the **only** post-creation write to the column, and it writes `false` unconditionally; `PATCH /api/admin/team/:id` whitelists `full_name, title_id, tier, jobber_user_id` (`team.js:274`) and does **not** accept `active`. **No route in the codebase can set `active = true`.** An Owner who deactivates the wrong person cannot undo it without a direct `UPDATE` in the Railway console. Taken with the frozen-rep note above, Decision E inherits **two** facts, not one: deactivation today is not a lifecycle *state* — it is a one-way door with a partial lock, where the person may still hold a working homeowner door and nobody can let them back in. **Recorded, deliberately not fixed here** — the control belongs to E, which is scoped to rule on reassignment and divvy alongside it.
  ⚠ **CORRECTED 2026-09-17 (Canvass-2) — THE BULLET ABOVE IS NOW *INVERTED*, NOT MERELY STALE, AND IT IS LEFT UNEDITED AS THE RECORD OF WHAT WAS TRUE ON THE DAY.** `PATCH /api/admin/team/:id/reactivate` exists under `requirePermission('team.manage')` and writes `UPDATE team_members SET active = true` inside a transaction — **verified in source, not carried from a report.** It shipped in C/DL-3c Phase 2c and closed on `PRE_LAUNCH_CHECKLIST.md` 2026-08-31. **So *"No route in the codebase can set `active = true`"*, the four-field allowlist remark and the Railway-console remark were all true when written and are all false now.** What makes this worth correcting rather than leaving to age is that the bullet **instructs Decision E to build something that is already there**. ⚠ **Its `team.js:555` and `team.js:274` citations have ALSO rotted and are DELIBERATELY NOT RENUMBERED** — a line number repaired inside a sentence whose claim is false certifies the wrong thing as fixed. Cite the deactivate handler and `PATCH /api/admin/team/:id` **by role**.
...
- ~~**Rep flags into React context.** `useAdminPermissions.js:40` builds a five-key object from the server response and silently discards `is_field_rep`, `is_attributable`, `rep_revenue_visibility` (and `title_id`). **The endpoint already selects and returns all three** — no server change needed. Six consumers, all destructuring named subsets, so the change is additive and low-risk; two test fixtures (`LockedSection.test.jsx:259`, `AdminTeamSettings.test.jsx:117-123`) assert shape parity and must be widened.~~ ✅ **DONE in C/DL-3c Phase 2a.** The prediction held on the part that mattered: the change was additive and the six consumers needed nothing. ⚠ **TWO THINGS IN THIS BULLET WERE WRONG AND ARE RECORDED RATHER THAN QUIETLY DROPPED, because both would have cost the next reader time.** (1) **`LockedSection.test.jsx:259` was a rotted citation** — `:259` is a lock-icon colour assertion; the `AdminPermissionsContext` fixtures are in that file's `PermissionGate` block, cited **by role** here because line numbers in this pair have already drifted once. Its sibling `AdminTeamSettings.test.jsx:117-123` was **correct** — ⚠ **one member of a pair right and one wrong, so no single delta could ever have repaired both.** (2) **Neither is a shape-parity ASSERTION.** Both merely *construct* partial context values as fixtures, so widening was additive for them too and neither had to change. What did change in `AdminTeamSettings.test.jsx` was unrelated: its PATCH fixture mirrored one server error string and was widened to all three rep flags.
