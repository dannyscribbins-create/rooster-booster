...
- **📌 TWO VACUITY FINDINGS IN TWO CONSECUTIVE SLICES, AND NEITHER WAS FOUND BY READING.** 6A's — case rows that pass before the field exists — was found by **injection**. 6B's — an assertion against a state that cannot show the value — was found by **driving the component to that state**. Both were invisible to inspection, and both would have shipped as green. The common lesson: **a test's own greenness is not evidence that it tests anything; only forcing the failure is.**
