...
| F9 | ~~HIGH~~ | B | `server/routes/stripe.js` (module scope) | A module-level constant held the *ghost* contractor id — not even the renamed one. Cited as `:13` until 2026-08-29; the line number is **not** updated because **the subject no longer exists**. | ✅ **RESOLVED 2026-08-29, Wave 1.1-e.** All five reads resolve from the caller's session; the constant is **deleted**, not left unused. Covered by `server/test/stripeContractorResolution.test.js`. |
| F10 | **🔴 LIVE — BROKEN NOW, NOT LATER** | B | `server/routes/account.js:436` | `SELECT email_sender_name, company_name FROM contractor_settings WHERE contractor_id = 'accent-roofing'` — the **PHANTOM** id. `contractors` holds exactly ONE row and it is `accent-roofing-dev`, so **this query returns ZERO ROWS TODAY**. The sender identity it supplies is **silently falling back on every email on that path, in production, right now.** "Hardcoded literal" understated it: this is not a future multi-tenant hazard, it is a current defect with no symptom anyone sees. | OPEN — owned by the contractor-ID reconciliation session; indexed in `PRE_LAUNCH_CHECKLIST.md` |
