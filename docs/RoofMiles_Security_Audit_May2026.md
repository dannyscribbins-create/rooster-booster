...
- CLAUDE.md character limit management — 31,350 chars before ABR 6A (2026-08-20); the performance-warning threshold is **40,000**, not 38K. ⚠ **CORRECTED 2026-08-23: THE 40,000 FIGURE IS WRONG AND THIS LINE IS ITS ORIGIN.** There is no fixed threshold — Claude Code's warning scales with the model's context window (2.1.169) and is counted in TOKENS, while every figure ever compared against it here was a BYTE count labelled "chars". Its consequence is a console warning, not truncation. ⚠ This line was inserted on 2026-08-21 by `d0fb3aa` — it is the only post-May content in a document titled "May 2026", which is what gave an unsourced number the appearance of audit provenance. It was copied into three further documents that then read as independent confirmation. → `CLAUDE.md` *Where New Content Goes*, and `PRE_LAUNCH_CHECKLIST.md`. Scoped rules in `.claude/rules/` spend none of that budget. Route new content by the protocol in CLAUDE.md's *Where New Content Goes*, never by size — and when the budget binds, reclaim reference rather than compressing a rule to fit.
