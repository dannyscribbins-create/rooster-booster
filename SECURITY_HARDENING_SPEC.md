...
**STATUS 2026-09-15 (second update) — THE URL-CONTEXT HALF IS NOW CLOSED TOO, on a gap this finding does not name.** Escaping stopped the attribute BREAKOUT and never stopped the SCHEME: nothing in `javascript:alert(1)` needs escaping, so an escaped hostile scheme landed in `src=`/`href=` perfectly intact and was still a link that runs code. ⚠ **A DIFFERENT CONTROL FOR A DIFFERENT HOLE, not defence in depth.** `buildEmailHtml()`'s logo, five socials and CTA now go through `safeLogoUrl`/`safeWebsiteUrl`, and so do the three `<img src>` in `pendingReferral.js`. ⚠ **THE LANDING PAGE HAD ALREADY SOLVED THIS FOR THE SAME COLUMNS** — those two functions moved verbatim to `server/utils/safeUrl.js` and all three callers share them, rather than a third opinion being written. ⚠ **AND `pendingReferral.js` HAD A LOCAL `const safeLogoUrl = escapeHtml(...)` — ESCAPING ONLY, NO SCHEME CHECK, A NAME CLAIMING A PROPERTY IT DID NOT HAVE.** It read as solved at all three call sites; it was found by sweeping for the SHAPE (a value inside a URL attribute), never for the name. Fenced by `server/test/emailUrlSafety.test.js`.
