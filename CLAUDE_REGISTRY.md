...
- Flipping `team_members.is_attributable` never touches existing `client_rep_assignments` rows in either direction — no retroactive sweep, forward or backward. It only changes what the attribution engine does starting from the next event. This falls out structurally rather than needing special-case code: the PATCH endpoint only ever writes the boolean column itself, and the engine's `is_attributable = true` checks are always evaluated live against `team_members` at event time — never cached or copied onto `client_rep_assignments`.
