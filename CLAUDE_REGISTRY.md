...
**Flagged Assignments Queue + Attributable Toggle (FA session, 2026-07-17–2026-07-19)**
- Files: server/migrations/add_flagged_assignments_status.js, server/migrations/widen_sticky_source_check.js, server/utils/attributionEngine.js, server/routes/admin/team.js, server/db.js, src/components/admin/AdminTeamSettings.jsx, src/components/admin/AdminFlaggedAssignmentsQueue.jsx, src/components/admin/AdminInboxSidebar.jsx, src/components/admin/AdminSettings.jsx, src/components/admin/AdminApp.jsx, src/components/admin/AdminComponents.jsx. Full spec: FLAGGED_ASSIGNMENTS_TOGGLE_SPEC.md. Locked assignment rules: docs/ASSIGNMENT_RULES_LOCKED.md.
- flagged_assignments gained a status lifecycle (open/resolved/dismissed/auto_resolved) replacing the boolean reviewed/reviewed_at columns (backfilled then dropped — see Architecture Notes for the column-existence guard nuance this surfaced). Engine dedup checks in writeCoAssignmentFlag/writeOrphanFlag now key off status='open' instead of reviewed=false.
- GET/PATCH /api/admin/team/flagged-assignments(/:id) — list (status filter, default open; hydrates reps_involved ids into {id, full_name}) and resolve (assign/dismiss), gated by the pre-existing rep_assignment permission flag — no new flag invented (FQ-2). The resolve PATCH's tenant+state guard lives on ONE claim-and-resolve UPDATE (WHERE id=$n AND contractor_id=$n AND status='open'), not a separate lookup — guard-proven by temporarily dropping the contractor_id predicate (confirmed RED: a cross-tenant write succeeded) then restoring it.
- Resolve-assign writes client_rep_assignments with sticky_source='manual' (new enum value in the sticky_source CHECK) and always supersedes any existing sticky (Decision B rule #4) — no branch cascade, since referral inheritance doesn't exist yet (vacuously faithful; see docs/ASSIGNMENT_RULES_LOCKED.md, "FA session scoping note").
- Auto-resolution (§4.5): writeSticky() closes any OPEN flag on the same (contractor_id, jobber_client_id) to auto_resolved whenever it actually sets a sticky, checked via the write's own rowCount (not just "was called") — a no-op write against an already-stickied client never auto-resolves anything.
...
- is_attributable toggle added to the existing PATCH /api/admin/team/:id field whitelist. AT-1 (binding rule — future-only semantics): see Architecture Notes. ⚠ **SUPERSEDED — this mechanism is no longer accurate (corrected 2026-08-21).** C/DL-3a Phase 2A **removed** `is_attributable` from that whitelist; `PATCH /api/admin/team/:id` now returns **422** naming the replacement (`team.js:243-247`), and `POST /api/admin/team/:id/promote` (`team.js:340`) is the **sole writer** of all three rep flags. The reason it was moved is this entry's own sibling: the PATCH could set `is_attributable=true` on a member with `is_field_rep=false`, which is exactly the write that produced Known Issue 13's production drift. **The FA session's work is accurately described above; only the endpoint has moved since.** See ground truth §E1.
...
**1. Scheduler possibly never registered — ✅ (a) RESOLVED (2026-08-21). Remainder SPLIT to #16 (Session 92 finding)**
- ⚠ **THIS ENTRY BUNDLED FIVE CONCERNS. Only the registration question is closed here.** The other four — the disconnect silence, (b), (c) and (d) — are **NOT resolved** and now live at **Known Issue 16**. Closing this entry whole would have taken them with it.
- **(a) `startCronJobs()` IS invoked in production — CLOSED, on three independent confirmations:**
  1. The CRM Token Fix (TF) session's clean-boot verification.
  2. C/DL-3b Phase 4's boot check — "cron registers fully."
  3. A production `sync_state` read on **2026-08-21** returning `accent-roofing-dev` with `last_synced_at` current to that day (Danny). A cron that never registers cannot produce a fresh timestamp.
- *Original finding (Session 92), preserved:* `POST /api/admin/crm/disconnect` deletes the tokens row (`admin/index.js:1101`); `runScheduledSync` (`crm/pipelineSync.js:793`) guards on `SELECT DISTINCT contractor_id FROM tokens WHERE access_token IS NOT NULL` and skips with only `[scheduler] No contractors with tokens — skipping cycle`. HOWEVER live deploy logs (July 2 18:28 EDT deploy, multiple :00/:30 UTC boundaries elapsed) show ZERO [scheduler] lines of ANY kind — neither skip nor start — suggesting `startCronJobs()` may not actually be invoked from the production entry point. Verification pending. Fixes pending review: (a) confirm/repair cron registration, (b) prominent skip logging, (c) sync-staleness alert via logError when `sync_state.last_synced_at` exceeds threshold, (d) launch consideration: contractor OAuth disconnect silently halts their pipeline sync with no recovery signal.
- ⚠ **The zero-`[scheduler]`-lines observation was never explained**, only outlived. It is consistent with (b) — skip logging too quiet to notice — which is why (b) is in #16 rather than closed here. **Do not read this closure as "the log question was answered."**
- Note the discovery query has since gained a second predicate (`crm/pipelineSync.js:900`): `JOIN contractors c ON c.id = t.contractor_id WHERE ... AND c.status = 'active'`. A contractor row whose `status` is not `'active'` is now silently skipped too — a second, newer instance of the same silence #16 covers.
...
- ~~`contractor_settings`: **split-brain** — 1 row under `contractor_id='accent-roofing'` and 1 row under `'accent-roofing-dev'`.~~ **✅ CLOSED 2026-08-29 (Wave 1.1-e). THE CLAIM WAS STALE AND THE VERIFICATION IT ASKED FOR HAD ALREADY BEEN DONE.** The `SELECT` this entry called for was run: `contractor_settings` holds **exactly one row** — the renamed tenant, `acct_1TUQ508MswQN98EW`, `active`. There is no ghost row and no merge to perform, so the proposed delete-then-rename / `COALESCE` fix is moot.
  ⚠ **THE DEFECT HERE WAS THE RECORD, NOT THE DATA.** A *"Pending: Danny to run a comparison SELECT"* that was completed and never written back is the same class as the unsourced-number and mis-pointed-check failures in `CLAUDE.md` — a mechanism reporting a state it stopped observing. It survived long enough that Wave 1.1-e Phase 0 raised it as a blocking open question and designed around it. **When a Known Issue names a pending verification, closing the verification means editing this line, not only knowing the answer.**
  **What it changed, recorded because the two failure modes look identical from the admin panel:** the card's *"not connected"* was the **manufactured fallback** — zero rows in, `|| { … 'not_connected' }` out — not a read of a phantom row; and pressing Connect Stripe would have **INSERTed** a phantom rather than updating one. `server/test/stripeContractorResolution.test.js` deliberately seeds the ghost row anyway, so the suite covers the split-brain state whether or not production is in it.
...
**⚠ 2a STILL OPEN — "STRIPE IS TENANTED NOW" MUST NOT READ AS "THE GHOST ID IS GONE" (Wave 1.1 close-out, 2026-08-30)**
- Wave 1.1-e (`08b2fc0`) resolved the contractor from the **session** across the whole Stripe surface, and the `contractor_settings` split-brain above is closed. **That is the STRIPE surface only.** The wider hardcoded-`'accent-roofing'` literal surface is untouched, and reading 2a's resolutions as a general closure is the specific misreading this note exists to prevent.
- **Measured 2026-08-30, `'accent-roofing'` literal in non-test `server/**`: 27 occurrences across 8 files** — `server/db.js` (**17**: 6 `DEFAULT 'accent-roofing'` column defaults, plus the schedule-backfill block and the seed guard at `:1144`/`:1583`) · `server/utils/notificationEmail.js` (4) · `server/utils/stripeTransfer.js` · `server/routes/webhooks/jobber.js` · `server/routes/account.js` · `server/referralRules.js` · `server/middleware/errorLogger.js` · `server/crm/jobber.js` (1 each).
  ⚠ **THE LAST THREE FILES ARE NOT IN THE LIST THIS ENTRY WAS DRAFTED FROM** — `stripeTransfer.js`, `webhooks/jobber.js` and `referralRules.js` were absent from it, which is exactly the hand-maintained-FILES-list failure `CLAUDE.md` records. **Re-derive with a grep; never work from this list without re-running it.**
- `server/routes/account.js:436` remains **live-broken** — it queries the phantom id by literal. → Contractor-ID reconciliation, Wave 1.4.
- **Also still open, and none of it is Stripe:** the **12 remaining untenanted gated handlers** → Wave 2.3 tenancy sweep · the **17 discard-form `verifyAdminSession()` call sites**, whose structural fix is **making the capture form the only form** rather than 17 patches (*fix by routing, not by replacing the value*) · **`payout_announcements` has no `contractor_id` column** · **`activity_log` has no `contractor_id` column, so match-jobber's two writes cannot be tenanted at all** — that one is a schema gap, not a missed predicate, and no sweep can close it.
...
- `CONSERVATIVE_REQUESTED_COST = 8055` in `runIncrementalSync` (`crm/pipelineSync.js`) and the `requestedQueryCost`/`actualQueryCost` figures behind the throttle-pacing design were calibrated via live GraphiQL against Jobber's **default API version (2025-04-16)**, not the pinned production version (`2026-05-12`, and `2026-02-17` when this note was written) — the version header did not apply on those calibration runs. The fix (throttleStatus-driven pacing + `clients(first: 25)`) is version-shape-agnostic and functions correctly regardless, but the exact constant and the "customFields contributes only ~150 points" / "quotes(first:10) truncation risk is theoretical" conclusions are provisional pending a re-run of the same calibration queries under `2026-02-17`. Re-run and update/delete this note once confirmed.
...
**14. Missing rep-promotion write-path — ✅ RESOLVED (verified 2026-08-21). Root cause of #13 (FA session finding, 2026-07-19)**
- **RESOLVED — BOTH HALVES, and neither was recorded as built.** ⚠ This entry read *"Do not build until: Explicitly scheduled by Danny"* while the work was already live. The original finding, preserved below, remains an accurate description of the state on 2026-07-19.
- **The write-path exists:** `POST /api/admin/team/:id/promote` (`server/routes/admin/team.js:340`, gated by `requirePermission('rep_promotion')`) is the **sole writer** of `is_field_rep`, `is_attributable` and `rep_revenue_visibility` (`:406-409`). It enforces coherence on the MERGED state (`:385-399`): a request that would set a dependent flag without `is_field_rep` is rejected, and turning `is_field_rep` off **cascades** both dependents to false in the same write. `PATCH /api/admin/team/:id` now returns **422** on `is_attributable` rather than silently ignoring it (`:243-247`).
- **The DB constraint exists too** — this entry asked only that one be "considered." `server/db.js:1672`: `CHECK (is_field_rep OR (NOT is_attributable AND NOT rep_revenue_visibility))`, wrapped in a work-remaining guard at `:1653` that fails the boot closed if any pre-existing row violates it, with operator instructions and a `CLAUDE_REGISTRY.md Known Issue 13` citation in the error text (`:1659-1662`). **#13's drift is now structurally impossible, not merely unlikely.**
- Verified by direct source read, 2026-08-21 — `docs/GROUND_TRUTH_2026-08-21.md` §E1.

*Original finding (2026-07-19), preserved:*
...
**16. Scheduler observability + disconnect silence — SPLIT OUT of Known Issue 1 (2026-08-21)**
- Known Issue 1 bundled five concerns under one heading. Its registration question — "is `startCronJobs()` actually invoked in production?" — is **CLOSED** (see #1). **These four are not, and they would have died inside a closed entry.**
- **The disconnect silence (the original headline).** `POST /api/admin/crm/disconnect` deletes the tokens row (`admin/index.js:1101`); `runScheduledSync` then skips that contractor with only `[scheduler] No contractors with tokens — skipping cycle`. A contractor who disconnects OAuth silently halts their own pipeline sync and **nothing tells anyone** — not them, not the admin, not `error_log`.
- **(b) Prominent skip logging.** The skip path logs at `console.log` level with no contractor attribution and no `logError`. A skipped cycle and a healthy cycle look the same in Railway logs unless you are reading closely.
- **(c) Sync-staleness alert via `logError` when `sync_state.last_synced_at` exceeds a threshold. ⚠ THIS IS NOT A NICE-TO-HAVE.** A staleness alert on `last_synced_at` is **precisely the mechanism that would have made (a) self-answering** — its absence is why "is the cron registered at all?" sat open from Session 92 to 2026-08-21, settled in the end by a manual database read. See `CLAUDE.md`, Test Design: *"A mechanism that reports health it cannot observe is worse than no mechanism."* **This is the inverse — the absence of any mechanism reporting a state that matters.**
- **(d) Launch consideration.** A contractor's OAuth disconnect silently halts their pipeline with **no recovery signal**. Under the 2026-08-21 launch definition (contractor-#2 readiness IS the launch gate), a self-serve contractor who disconnects and cannot tell that anything broke is a launch-visible failure, not a post-launch one.
- ⚠ **Why this is a split and not a re-open:** #1's five concerns shared a heading and nothing else. Closing the one that was answered would have taken the other four with it — the closure-half defect, applied to the registry itself. Recorded here so the remainder survives #1's closure.
- Do not build until: its own scoped session. **(c) first** — it is the cheapest and it is what makes the rest observable.

**17. `user_badges` is LIVE, and its milestone counter counts the wrong thing (Wave 1.2 Phase 0, 2026-08-30)**
- ⚠ **APPENDED, NOT INSERTED. Nothing was renumbered.** Section numbers in this file are **load-bearing at runtime**: `server/db.js:1662` cites *"CLAUDE_REGISTRY.md Known Issue 13"* inside a production `console.error`, and 13 further code sites cite #13 or #2a (`db.js:1630`, `routes/admin/team.js:232`, `routes/stripe.js:55`, `utils/stripeTransfer.js:12`, `src/components/admin/AdminTeamSettings.jsx:915`, and 7 test files). **Nothing cites #16 or #17, so appending shifts nothing — the hazard is renumbering or inserting, and this entry did neither.**
- ⚠ **`user_badges` IS NOT A DORMANT EARLY-GAMIFICATION ARTIFACT. IT IS LIVE, AND THE RANK ARC MUST BE TOLD SO EXPLICITLY.** **3 writers** — `server/routes/referrer.js:264`, `:526`, `server/routes/admin/referrers.js:89`. **4 readers** — `referrer.js:244`, `:2536`, `:2727`, `admin/referrers.js:196`. **2 rendered components** — `src/components/referrer/BadgeCelebrationPopup.jsx` and `ProfileTab.jsx`, plus the leaderboard's `display_badge`.
  **Why this warning exists rather than a description of a working table:** *"a badge table already exists"* plus a counter that means something else is exactly how a rank ladder inherits a count that is **wrong by the pipeline's conversion rate**. The RANK arc will encounter this table while building a five-rung ladder over the same users. **Leave it alone; build rank on `referral_conversions`.**
- **THE MECHANISM (the defect itself is tracked as open work in `PRE_LAUNCH_CHECKLIST.md` → *Milestone badges count pipeline rows* — cross-linked, not duplicated).** `server/routes/referrer.js:908` calls `checkAndAwardBadges(userId, data.pipeline.length)`. **`data.pipeline.length` is every pipeline row — leads, inspections, sold, closed, all of it.** The candidates at `:251-255` (`first_referral`, `milestone_5`, `milestone_10`, `milestone_25`) are named for referrals and gated on that total. **A referrer with 5 leads and 0 paid jobs holds `milestone_5` in production today.**
- **Two schema facts that constrain any fix.** `user_badges` (`server/db.js:137-144`) is `id, user_id, badge_id, earned_at, seen`, `UNIQUE(user_id, badge_id)`:
  - **No `contractor_id`.** Every read and write is keyed on `user_id` alone. Any reuse of this table on a tenant-scoped surface imports a tenancy hole.
  - **No revocation path.** Insert-only, `ON CONFLICT DO NOTHING`, and nothing anywhere deletes a row. **Anything already granted stays granted whatever the fix** — so the fix is forward-only and the existing false milestones are a separate, deliberate decision.
- Do not build until: its own scoped session, or folded into whichever session rules on what a conversion row means (see the checklist's *read endpoint writes a payout row* entry — the two share a root cause in what "a referral" counts as).
...
**Migration guard idempotency — refinement: column-existence guards for drop-and-replace migrations (FA session, 2026-07-18)**
- The ST-session pattern above (guard on "any rows with the new column still NULL") is necessary but not sufficient when the migration's backfill statement references a legacy column BY NAME — once a prior invocation has already dropped that column, the row-based guard is satisfied (zero NULL rows) but the backfill UPDATE itself still fails at parse/plan time with "column does not exist," because the guard never checked whether the column it's about to reference still exists.
- Applied in `add_flagged_assignments_status.js`: the backfill-then-drop block (copies `reviewed`/`reviewed_at` into `status`/`resolved_at`, then drops both) is gated on `IF EXISTS (SELECT 1 FROM information_schema.columns WHERE column_name = 'reviewed')` — a permanent no-op once that column is gone, regardless of row state. Discovered via the same discipline the ST pattern recommends: a fresh-schema double-run test caught it immediately (second invocation threw "column reviewed does not exist").
- Rule for future drop-and-replace migrations: if a step's SQL names a column being dropped elsewhere in the same migration, gate that step on the column's existence, not merely on whether backfill work remains.

**is_attributable toggle — future-only, binding rule (AT-1, FA session, 2026-07-19)**
- Flipping `team_members.is_attributable` never touches existing `client_rep_assignments` rows in either direction — no retroactive sweep, forward or backward. It only changes what the attribution engine does starting from the next event. This falls out structurally rather than needing special-case code: **the writing endpoint only ever writes the boolean column itself**, and the engine's `is_attributable = true` checks are always evaluated live against `team_members` at event time — never cached or copied onto `client_rep_assignments`.
- ⚠ **MECHANISM CORRECTION (2026-08-21): the endpoint named above is no longer the PATCH.** This bullet said "the PATCH endpoint" until C/DL-3a Phase 2A moved the write. `POST /api/admin/team/:id/promote` (`team.js:340`) is now the sole writer of all three rep flags, and `PATCH /api/admin/team/:id` **rejects** `is_attributable` with a 422 (`team.js:243-247`). **AT-1 ITSELF IS UNCHANGED AND STILL BINDING** — only the name of the endpoint that performs the write has moved. The structural argument holds identically: promote writes the boolean column and nothing else, and the engine still evaluates live at event time.
- Do not add a retroactive sweep (forward or backward) for this flag without an explicit, separately-scoped decision from Danny — AT-1 is locked as future-only.
