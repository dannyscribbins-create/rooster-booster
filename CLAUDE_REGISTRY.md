...
**2c. Token-refresh coordination gap — mitigated, durable fix deferred (Session 94, 2026-07-06)**
- Root cause of a chronic live 401 (`error_log` source `POST /webhooks/jobber/invoice-paid — fetchInvoiceWithJobs`, first_seen 2026-06-23, count 241+): Jobber refresh-token rotation is enabled (exchanging a refresh token invalidates the prior one immediately), and `refreshTokenIfNeeded()` (`server/crm/jobber.js`) operates on a single `tokens WHERE id=1` row refreshed independently from ~7 uncoordinated call sites (invoice-paid webhook, 30-min pipeline sync cron ×2, `fullJobberImport.js` ×2, `admin/campaigns.js`, `admin/team.js`, `referrer.js`) with no locking. Confirmed live: two invoice-paid webhooks arriving the same second produced two "Refreshing token..." log lines but only one "Token refreshed" — the loser's request then hit a 401 against Jobber's GraphQL API.
- **MITIGATED**: `refreshTokenIfNeeded(force)` now accepts an optional `force` param (default `false`, fully backward-compatible — no other call site passes it). The invoice-paid webhook's `fetchInvoiceWithJobs` call catches a 401 specifically, force-refreshes, re-reads the token row, and retries exactly once with the freshly-read value (also reused for the subsequent `fetchFullClient` call in the same handler). A non-401 error does not trigger a forced refresh. Covered by `server/test/invoicePaidWebhook.test.js`.
- **STILL DEFERRED**: the durable fix — single-flight/mutex-guarded refresh (or per-contractor token keying) so the ~7 call sites can't race at all — is a bigger, riskier change touching a function shared across several files. Not built this session; the retry mitigation above is the correct minimal fix for the specific 401 pattern observed, but the underlying race is still possible under different timing.
...
**D5 — two-stage gate on contractor #2 (binding, ties to the separate CRM Token Fix effort):** S3's checkpoint makes a second `contractors` **row** resolution-safe — webhook and referrer traffic both resolve tenancy dynamically now, so inserting a second row no longer breaks Accent. A second contractor's Jobber OAuth **connect**, however, remains **FORBIDDEN** until the CRM Token Fix (TF) session deploys and verifies: `oauth.js`'s token upsert is still hardcoded to `tokens.id=1` (F2, tracked separately, out of this rebuild's scope) — the moment a second contractor clicks Connect, that upsert overwrites Accent's row and destroys Accent's live token. Accent itself must also not reconnect/re-authorize Jobber until TF ships, for the same reason (the OAuth-time `jobber_account_id` capture added in S3 would otherwise be the thing that triggers the overwrite).

**Next:** the F2/F3/F4/F5 CRM Token Fix (TF) — the `tokens.id=1` hardcode and tenant-blind `refreshTokenIfNeeded()` — is the remaining blocker before contractor #2's Jobber connect is safe. Not started.
