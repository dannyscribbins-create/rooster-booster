...
**Next:** none — the F2/F3/F4/F5 CRM Token Fix (TF) was the last blocker before contractor #2's Jobber connect is safe. Contractor #2 onboarding can proceed pending the deferred cross-tenant `users`-matching item ([[Known Issues — users matching in invoice-paid webhook and pipelineSync]]) and the "STILL OPEN" literal sweep in [[Known Issues 2a]].
...
- Execution Plan v1 docx amendment: strike "(F2/F3/F4/2c)" from the B1-A S2 line; B1-A now reads S1 · S2 · S3 · TF complete. The docx lives outside the repo — Danny owns this edit at the roadmap reconciliation session.
